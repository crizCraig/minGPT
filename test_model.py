...
def main():
    ...
    model = GPT.load_from_checkpoint('/home/c2/src/minGPT/checkpoints/lightning_logs/version_24/checkpoints/epoch=1-step=29033.ckpt',
                                     vocab_size=65, block_size=128, n_layer=8, n_head=8, embedding_dim=256)
    x = torch.tensor([train_dataset.stoi[s] for s in input_prompt], dtype=torch.long)[None, ...]
    if next(model.parameters()).is_cuda:
        x = x.cuda()
    y = sample(model, x, 500, temperature=1.0, sample=True, top_k=None)[0]
